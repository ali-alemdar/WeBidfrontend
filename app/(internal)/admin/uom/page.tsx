...
export default function AdminUomPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await apiGet(`/uom/admin${params.toString() ? `?${params}` : ""}`);
      setRows(Array.isArray(r) ? r : []);
    } catch (e: any) {
      setError(e?.message || "Failed to load UOM");
    } finally {
      setLoading(false);
    }
  };
  ...
  const add = () =>
    doAction("add", async () => {
      ...
      await apiPost("/uom", { code: code.trim(), name: name.trim() || null, isActive: true });
      setCode("");
      setName("");
      setCreateOpen(false);
    });
  ...
  return (
    <RequireRoles anyOf={["SYS_ADMIN"]} title="Admin / Units of Measure">
      <InternalPage title="Admin / Units of Measure">
                ...
                <tr key={u.id} style={{ borderTop: "1px solid var(--border)" }}>
                  ...
                  <td style={{ textAlign: "right" }}>
                    <button
                      className="btn"
                      disabled={acting !== ""}
                      onClick={() => doAction(`toggle-${u.id}`, () => apiPut(`/uom/${u.id}`, { isActive: !u.isActive }))}
                    >
      ...
      {createOpen && (
        ...
        >
          ...
          >
            ...
            <label style={{ display: "block", marginBottom: 10 }}>
              <div style={{ fontWeight: 800, marginBottom: 6 }}>Name (optional)</div>
              <input
                className="input"
                value={name}
