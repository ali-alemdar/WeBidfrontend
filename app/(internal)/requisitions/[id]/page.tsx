...
export default function RequisitionDetailPage({ params }: Props) {
  ...
  const {
    ...
    scope: "EDIT_PAGE",
    editUrl: `/requisitions/${params.id}/edit`,
    heartbeatUrl: `/requisitions/${params.id}/edit-heartbeat`,
    releaseUrl: `/requisitions/${params.id}/edit-release`,
    forceReleaseUrl: `/requisitions/${params.id}/force-release-lock`,
    mapEditResponse: (res: any) => ({
      data: res?.requisition ?? res ?? null,
  ...
  const load = async () => {
    ...
    try {
      // Always load dropdown master data (departments, categories, UOM)
      ...
      const u = res[1];
      const c = res[2];
      const s = canEditMasterData ? res[3] : [];
      const discT = canEditMasterData ? res[4] : [];
      const bidT = canEditMasterData ? res[5] : [];
  ...
  // When the lock context (editData) changes, hydrate local requisition state
  useEffect(() => {
    if (!editData) return;

    const r = editData as any;
    ...
    // If requisition already has a disclaimer, keep it; otherwise use existing default.
    if (!isRequesterOnly) {
      setDisclaimerText(
        r?.disclaimerText ||
          "For market price discovery only – no purchase commitment.",
    ...
    // After lock-backed data arrives, ensure master data is loaded.
    // We do not await here to avoid blocking render.
    // eslint-disable-next-line @typescript-eslint/no-floating-promises
    load();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [editData, isRequesterOnly, params.id]);
  ...
  const isLockedByAnother = lockStatus !== 'OWNED';
  ...
  if (!data) {
    return (
      <RequireRoles anyOf={["REQUESTER", "REQUISITION_OFFICER", "REQUISITION_MANAGER", "TENDERING_OFFICER", "TENDER_APPROVAL", "SYS_ADMIN"]} title={`Requisition ${params.id}`}>
        <InternalPage title={`Requisition ${params.id}`}>
          {combinedError ? <p style={{ color: "#b91c1c" }}>{combinedError}</p> : <p>Not found.</p>}
        </InternalPage>
      </RequireRoles>
    );
  }
        ...
        {lockStatus !== 'OWNED' && lockInfo && (
          ...
          >
            ...
            {isSysAdmin && (
              <button
                onClick={async () => {
                  if (confirm('Are you sure you want to force-release this lock? The other user will lose their lock.')) {
                    try {
                      await forceRelease?.();
                      try {
                      ...
                      try {
                        // eslint-disable-next-line @typescript-eslint/no-var-requires
                        const { toast } = require('react-hot-toast');
                        toast?.error?.(e?.message || 'Failed to release lock');
                      } catch {
                        // eslint-disable-next-line no-console
