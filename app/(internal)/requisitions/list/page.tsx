...
export default function RequisitionListPage() {
  ...
  const closingStats = useMemo(() => {
    ...
    const sourceRows = isSysAdmin ? data.filter((r) => displayStatus(r.status) !== "DRAFT") : data;
  ...
  return (
    <RequireRoles anyOf={["REQUISITION_OFFICER", "REQUISITION_MANAGER", "TENDERING_OFFICER", "SYS_ADMIN"]} title="Requisitions">
      ...
      >
        ...
        <PieChart title="By target / closing" segments={closingSegments} />
      </div>
      ...
      <table
        cellPadding={8}
        style={{ borderCollapse: "collapse", tableLayout: "fixed", width: 1025 }}
      >
        <thead>
          <tr style={{ textAlign: "left" }}>
            ...
            <th style={{ width: 105 }}>Target</th>
            <th style={{ width: 105 }}>Created</th>
          </tr>
        </thead>
        <tbody>
          {filtered.map((r) => (
            <tr key={r.id} style={{ borderTop: "1px solid var(--border)" }}>
              ...
              <td style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.requestingDepartment || ""}</td>
