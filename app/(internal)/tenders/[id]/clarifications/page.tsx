...
export default function TenderClarificationsPage({ params }: Props) {
  ...
  const load = async () => {
    ...
    try {
      const [c, a] = await Promise.all([
        apiGet(`/tenders/${tenderId}/clarifications`),
        apiGet(`/tenders/${tenderId}/addenda`),
      ]);
  ...
  const answer = async (clarificationId: number) => {
    ...
    try {
      ...
      await apiPost(`/tenders/${tenderId}/clarifications/${clarificationId}/answer`, {
        answer: text,
      });
  ...
  const createAddendum = async () => {
    ...
    try {
      ...
      await apiPost(`/tenders/${tenderId}/addenda`, {
    ...
    <RequireRoles anyOf={["TENDERING_OFFICER", "SYS_ADMIN"]} title="Clarifications & Addenda">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <span className="pill">Tender: {tenderId}</span>
          {loading ? <span className="pill">Loading…</span> : null}
        </div>

        <div style={{ display: "flex", gap: 8 }}>
          <Link className="btn" href={`/tenders/${tenderId}`}>Back to Tender</Link>
