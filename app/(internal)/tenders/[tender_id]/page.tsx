...
export default function TenderDetailPage({ params }: Props) {
  ...
  const [allGMReturnNotes, setAllGMReturnNotes] = useState<any[]>([]);
  const [activeTab, setActiveTab] = useState<"prep" | "publishing" | "section8" | "documents">("prep");
  const [publishingDocuments, setPublishingDocuments] = useState<any[]>([]);
  ...
  const isGMReturned = data?.status === "TENDER_RETURNED_GM";
  const anyOfficerApproved = approvals.some((a: any) => a.decision === "APPROVED");
  const totalOfficers = approvals.length;
  const approvedCount = approvals.filter((a: any) => a.decision === "APPROVED").length;
  const allOfficersApproved = totalOfficers > 0 && approvedCount === totalOfficers;
  // TENDER_RETURNED_GM: read-only for TENDER_APPROVAL, editable for TENDERING_OFFICER
  const gmReturnedReadOnly = isGMReturned && isManager && !userRoles.includes("TENDERING_OFFICER") && !userRoles.includes("SYS_ADMIN");
  const canEdit = lockStatus === "OWNED" && !isIdle && !lockReleased && !isApprovalInProgress && !isArchived && !anyOfficerApproved && !gmReturnedReadOnly;
  ...
  const load = async () => {
    ...
    try {
      ...
      const [c, w, notes, approv, retNotes, rejNotes, gmRetNotes, pubData] = await Promise.all([
        ...
        apiGet(`/tenders/${params.tender_id}/gm-return-notes`).catch(() => ({ notes: [] })),
      ...
      setAllGMReturnNotes((gmRetNotes?.notes || []).reverse());
  ...
  const action = async (name: string, path: string, body: any = {}) => {
    ...
    try {
      ...
      // Reload data to refresh approvals
      await load();
      // Redirect to approval queue if manager approved, rejected
      if (isManager && ["managerApprove", "reject"].includes(name)) {
        setTimeout(() => router.push("/tenders/waiting-approvals"), 500);
      }
      // For return, stay on page to show updated state
    } catch (e: any) {
    ...
    <RequireRoles anyOf={["TENDERING_OFFICER", "TENDER_APPROVAL", "GENERAL_MANAGER", "SYS_ADMIN"]} title={`Tender ${tenderDisplay}`}>
      ...
      {lockStatus === "LOCKED" && lockInfo && (
        <div style={{ backgroundColor: "#fef3c7", color: "#92400e", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #fcd34d" }}>
          <strong>⚠️ Edit locked:</strong> This tender is currently being edited by{" "}
      ...
      {lockStatus === "OWNED" && !isIdle && !lockReleased && (
        <div style={{ backgroundColor: "#dcfce7", color: "#166534", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #86efac" }}>
          <strong>✅ Lock acquired:</strong> You are editing this tender. The lock will automatically release after 30 seconds of inactivity.
        </div>
      )}
      {lockReleased && (
        <div style={{ backgroundColor: "#fef3c7", color: "#92400e", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #fcd34d" }}>
          <strong>⚠️ Lock released due to inactivity:</strong> Please refresh the page to edit again.
        </div>
      )}
      ...
      {anyOfficerApproved && (
        <div style={{ backgroundColor: "#fef3c7", color: "#92400e", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #fcd34d" }}>
          <strong>✓ Officer Approval(s):</strong> {approvals.filter((a: any) => a.decision === "APPROVED").map((a: any) => a.fullName).join(", ")} approved this tender. No changes can be made unless their approval is cancelled.
        </div>
      )}
      ...
      {allReturnNotes.length > 0 && (
        <div style={{ backgroundColor: "#fef3c7", color: "#92400e", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #fcd34d" }}>
          <strong>📝 Manager Notes - Return for Changes</strong>
      ...
      {allRejectNotes.length > 0 && (
        <div style={{ backgroundColor: "#fee2e2", color: "#b91c1c", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #fca5a5" }}>
          <strong>❌ Manager Notes - Tender Rejected</strong>
          <div style={{ marginTop: 12 }}>
      ...
      {/* GM Return Notes - show at top */}
      {allGMReturnNotes.length > 0 && (
        <div style={{ backgroundColor: "#dbeafe", color: "#1e40af", padding: 12, borderRadius: 8, marginBottom: 12, border: "1px solid #93c5fd" }}>
          <strong>📝 General Manager - Returned for Changes</strong>
          {isGMReturned && gmReturnedReadOnly && (
            <span style={{ marginLeft: 12, fontSize: 12, color: "#6b7280" }}>(Read-only - awaiting officer updates)</span>
          )}
          <div style={{ marginTop: 12 }}>
            {allGMReturnNotes.map((note: any, idx: number) => (
              <div key={idx} style={{ marginBottom: 12, paddingBottom: 12, borderBottom: idx < allGMReturnNotes.length - 1 ? "1px solid rgba(0,0,0,0.1)" : "none" }}>
                <div style={{ fontSize: 12, color: "#666", marginBottom: 6 }}>
                  {note.createdAt ? new Date(note.createdAt).toLocaleString() : ""}
                </div>
                <div style={{ whiteSpace: "pre-wrap", fontFamily: "monospace", fontSize: 13 }}>{note.reason}</div>
      ...
      {data && (data.status === "TENDER_PREP_DRAFT" || data.status === "TENDER_PREP_RETURNED" || data.status === "TENDER_PENDING_APPROVAL" || data.status === "GM_APPROVAL_PENDING" || data.status === "TENDER_READY_FOR_PUBLISHING" || data.status === "TENDER_RETURNED_GM") && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12, borderBottom: "1px solid var(--border)", overflowX: "auto" }}>
          ...
          {data?.status === "TENDER_READY_FOR_PUBLISHING" && isManager && (
            <button
              ...
              style={{
                ...
                color: activeTab === "documents" ? "white" : "inherit",
                cursor: "pointer",
        ...
        <>
          ...
          {data && (isOfficer || isManager) && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
              ...
              {isOfficer && (data.status === "TENDER_PREP_DRAFT" || data.status === "TENDER_PREP_RETURNED" || data.status === "TENDER_RETURNED_GM") && 
              ...
              {/* Officer Cancel Approval Button - show if already approved, not manager approved, and not in GM_APPROVAL_PENDING */}
              {isOfficer && data.status !== "TENDER_PREP_APPROVED" && data.status !== "GM_APPROVAL_PENDING" && 
                approvals.find((a: any) => a.userId === user?.id && a.decision === "APPROVED") && (
                <button
                  className="btn"
                  ...
                  style={{ color: "#b91c1c" }}
                >
                  {acting === "cancel" ? "Cancelling..." : "✗ Cancel my approval"}
                </button>
              )}
              
              {/* Manager Approve Button - show if in pending approval or returned status */}
              {isManager && (data.status === "TENDER_PENDING_APPROVAL" || data.status === "TENDER_RETURNED_GM") && (
                <button
                  className="btn btn-primary"
                  onClick={() => action("managerApprove", `/tenders/${params.tender_id}/prep-approvals/manager-approve`, {})}
                  disabled={acting === "managerApprove" || lockStatus === "LOCKED" || lockReleased || !allOfficersApproved}
                  title={lockStatus === "LOCKED" ? "Cannot approve: page is locked by another user" : lockReleased ? "Cannot approve: lock was released due to inactivity" : undefined}
                >
                  {acting === "managerApprove" ? "Approving..." : "✓ Approve"}
                </button>
              )}
              
              {/* Manager Cancel Approval Button - DISABLED when status is GM_APPROVAL_PENDING */}
              
              {/* Manager Return Button - show if in pending approval or returned status */}
              {isManager && (data.status === "TENDER_PENDING_APPROVAL" || data.status === "TENDER_RETURNED_GM") && (
                <button
                  className="btn"
                  onClick={() => setShowReturnModal(true)}
                  disabled={acting !== "" || lockStatus === "LOCKED" || lockReleased || !allOfficersApproved}
              ...
              {/* Manager Reject Button - show if in pending approval or returned status */}
              {isManager && (data.status === "TENDER_PENDING_APPROVAL" || data.status === "TENDER_RETURNED_GM") && (
                <button
                  className="btn"
                      ...
                      <tbody>
                        {copy.items.map((item: any, idx: number) => (
                          <tr key={idx} style={{ borderTop: "1px solid var(--border)", fontSize: "12px" }}>
                            <td style={{ textAlign: "center" }}>{item.itemNo || "—"}</td>
                            <td>{item.description || "—"}</td>
                            <td>{item.dateOfDelivery || "—"}</td>
                            <td>{item.uom || "—"}</td>
                            <td style={{ textAlign: "center" }}>{item.quantity || "—"}</td>
                            <td style={{ textAlign: "left" }}>{item.unitPrice ? `${item.currency || ""} ${item.unitPrice.toLocaleString()}` : "—"}</td>
                            <td style={{ textAlign: "left" }}>{item.totalPrice ? `${item.currency || ""} ${item.totalPrice.toLocaleString()}` : "—"}</td>
                  ...
                  <table width="100%" cellPadding={8} style={{ borderCollapse: "collapse", minWidth: 1200 }}>
                    ...
                    <tbody>
                      {workingItems.map((item: any, idx: number) => (
                        <tr key={idx} style={{ borderTop: "1px solid var(--border)", fontSize: "12px" }}>
                          <td style={{ textAlign: "center", fontWeight: "bold", color: "#666" }}>{item.itemNo || "—"}</td>
            ...
            <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(2, 1fr)" }}>
              ...
              <label>
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "8px 12px", backgroundColor: "#f5f5f5", borderRadius: "4px", border: "1px solid #ddd" }}>
                  {publishingData.tender_doc_price ? `IQD ${new Intl.NumberFormat('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 }).format(publishingData.tender_doc_price)}` : "—"}
                </div>
              </label>
            </div>
          </div>
      ...
      {/* Documents Tab - for TENDER_READY_FOR_PUBLISHING */}
      {activeTab === "documents" && data?.status === "TENDER_READY_FOR_PUBLISHING" && isManager && (
        <>
          <div className="card">
            <h3 style={{ marginTop: 0 }}>Documents to Publish</h3>
            <p style={{ color: "#666", fontSize: 14, marginBottom: 16 }}>Select documents to include in publication. Check the documents you want to publish:</p>
            
            <div style={{ display: "grid", gap: 12 }}>
              {/* Bid Letter Document */}
              <div style={{ display: "flex", gap: 12, padding: 12, backgroundColor: "#f9fafb", borderRadius: 8, border: "1px solid #e5e7eb", alignItems: "flex-start" }}>
                <input
                  type="checkbox"
                  defaultChecked={true}
                  ...
                  onChange={(e) => {
                    setPublishingDocuments(prev => {
                      const idx = prev.findIndex((d: any) => d.id === "bid-letter");
                      if (idx >= 0) {
                        const updated = [...prev];
                        updated[idx].included = e.target.checked;
                        return updated;
                      }
                      return [...prev, { id: "bid-letter", name: "Bid Letter", included: true }];
                    });
                  }}
                />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, marginBottom: 4 }}>Bid Letter (PDF)</div>
                  <div style={{ fontSize: 12, color: "#666", marginBottom: 8 }}>Official bid invitation document signed by General Manager</div>
                  <button
                    className="btn"
                    style={{ fontSize: 12, padding: "4px 8px" }}
                    onClick={() => setPreviewDocument({ id: "bid-letter", name: "Bid Letter", type: "pdf" })}
                  >
                    👁️ Preview
                  </button>
                </div>
              ...
              <div style={{ textAlign: "center", padding: 24, color: "#9ca3af", backgroundColor: "#f3f4f6", borderRadius: 8 }}>
                <p>Additional documents can be uploaded and managed here</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
            ...
            <button className="btn" style={{ padding: "10px 16px" }}>
            ...
            <div className="card" style={{ marginBottom: 12 }}>
              ...
              <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(2, 1fr)" }}>
                <div>
                  ...
                  <div style={{ fontWeight: 600 }}>{publishingData.tender_title || "—"}</div>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: "#666" }}>Publication Tender Number</div>
                  <div style={{ fontWeight: 600 }}>{publishingData.publication_tender_number || "—"}</div>
                </div>
                <div style={{ gridColumn: "1 / -1" }}>
                  <div style={{ fontSize: 12, color: "#666" }}>Description</div>
                  <div style={{ fontWeight: 600, whiteSpace: "pre-wrap" }}>{publishingData.tender_description || "—"}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div style={{ fontWeight: 600 }}>{publishingData.opening_date || "—"}</div>
                </div>
              </div>
            </div>
          ) : (
            <div className="card" style={{ marginBottom: 12 }}>
              <h3 style={{ marginTop: 0 }}>No publishing data available</h3>
              <p>Publishing data has not been configured for this tender yet.</p>
            </div>
          )}
      ...
      {previewDocument && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: 16, borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, backgroundColor: "white" }}>
              <h3 style={{ margin: 0 }}>{previewDocument?.name} - Preview</h3>
              <button
                className="btn"
            ...
            <div style={{ flex: 1, overflow: "visible", backgroundColor: "#f3f4f6", padding: "20px" }}>
              {previewDocument?.id === "bid-letter" ? (
                <iframe
                  src={`${process.env.NEXT_PUBLIC_API_BASE || "http://localhost:3001"}/tenders/${data?.id || params.tender_id}/bid-letter-pdf`}
                  style={{
                    width: "100%",
                    height: "800px",
                    border: "none",
